...
class edit_data():
    ...
    def app(self):
        ...
        try:
            ...
            df = pd.read_sql('select * from SAMPLE2',conn)
            conn.close()
            ...
            if apply:
                ...
                if len(flag) == 0:
                    try:
                        try:
                            conn = Connect_Db.connectdb()
                            query = f"UPDATE sample2 SET DATE = '{date}', AMOUNT = {amount}, TRANSACTION_TYPE = '{trans_type}', CATEGORY = '{category_name}' WHERE INDEX = {index}"
